...
\begin{itemize}
    \item Indeks $i$ pada $a_{ij}$ dan $b_i$ menunjukkan baris ke-$i$ dalam sistem 
    yang ditampilkan, atau secara ekuivalen, persamaan ke-$i$.
    \item Indeks $j$ pada $a_{ij}$ menunjukkan kolom ke-$j$ yang berkaitan dengan 
    peubah ke-$j$, untuk $1 \leq j \leq n$.
\end{itemize}

\textbf{Definisi 1.2.5. Solusi Sistem Linear.} 
Sebuah \textit{solusi persamaan linear}
...
Dalam hal ini, kita katakan bahwa $(s_1, \ldots, s_n)$ \textit{memecahkan persamaan tersebut}.

Sebuah \textit{solusi sistem persamaan linear}
...
$(s_1, s_2, \ldots, s_n)$ \textit{memecahkan sistem tersebut}.